            ...
            <div class="layui-card">
                <div class="layui-card-body layui-form">
                    ...
                    <table class="layui-table">
                        ...
                        <tbody>
                        <#list channel.data as d>
                            <tr>
                                <td>${(d.id)!}</td>
                                <td>${(d.name)!}</td>
                                <td>${(d.createTime?string('yyyy-MM-dd HH:mm:ss'))!}</td>
                                <td><a class="layui-btn layui-btn-sm" lay-href="/channel/update?id=${(d.id)!}" lay-id="${(d.id)!}">修改</a>
                                    <a href="javascript:;" onclick="doDel('/channel/del?id=${(d.id)!}','删除')" class="layui-btn layui-btn-danger layui-btn-sm">删除</a>
                                </td>
                            </tr>
                        </#list>
                        </tbody>
                    </table>共${(channel.total)!}条
                    <@pager count="${(channel.total)!}" limit="${(channel.size)!}" curr="${(channel.current)!}" create="/channel/edit" />
                </div>
            </div>
        </div>
    </div>
